...
// client, even though identity resources are organization-scoped.
// NSCALE_TEST_ROLE_ID identifies a live, pre-configured role in the target
// organization; roles are read-only platform resources and cannot be created by
// the test, so group membership cannot be exercised without one.
func testAccPreCheck(t *testing.T) {
	...
	for _, v := range []string{
		...
		if os.Getenv(v) == "" {
			t.Skipf("%s must be set for identity acceptance tests", v)
		}
	}
}
