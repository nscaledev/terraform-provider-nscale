...
**Do not edit `CHANGELOG.md`.** It is generated by release-please from commit
subjects and regenerated on every push to `main` — see
[RELEASING.md](../../../../RELEASING.md). Your changelog entry *is* your PR
title, so write the title as the release note.

Pick the type deliberately, because it sets the version bump: a new resource,
data source, or attribute is `feat:` (MINOR) even when the diff is one line;
anything that breaks an existing configuration needs `feat!:` or a
`BREAKING CHANGE:` footer (MAJOR); `fix:` and `docs:` are PATCH; `chore:`,
`ci:`, `test:`, `style:` and `build:` are hidden from the changelog entirely,
which is what internal refactors should use.

Write the subject to the user's mental model, not the code's: `fix(objectstorage):
persist versioned = false correctly`, not `fix: bucketAPIRequest.MarshalJSON`. A
good subject answers what changed and whether the user needs to act; put the
"why" and any migration steps in the commit body, where the PR reviewer and
`git log` will find them.
