...
4. **Register** the resource/data source factory in `internal/provider/provider.go`. **Without this step the type compiles but is invisible to users.** It is the single most common oversight.
5. **Build:**
   ```sh
   make install
   ```
